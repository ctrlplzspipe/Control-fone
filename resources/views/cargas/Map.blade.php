...
@section('title', 'Carga de Plazas MAP - Control de Plazas')
                ...
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-xl sm:text-2xl font-extrabold text-gray-800 tracking-tight">
                                Carga de Estatus de Plazas Conciliadas en FONE (MAP)
                            </h1>
                            <p class="text-xs sm:text-sm text-gray-500 mt-0.5">
                                Módulo de actualización del estatus de plazas conciliadas.
                            </p>
                        </div>
                    </div>
                    ...
                    <div>
                        <span
                            ...
                            Último MAP Cargado: <strong class="text-gray-800 ml-1">{{ $fecha }}</strong>
                        </span>
                    </div>
                </div>

                <!-- Formulario -->
                <form id="formCargaMap" enctype="multipart/form-data" class="space-y-6 no-loader">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="lbl_FECHA"
                                ...
                                Fecha de Actualización <span class="text-red-500">*</span>
                            </label>
                            <input type="date" name="lbl_FECHA" id="lbl_FECHA"
                            ...
                            <p class="text-[11px] text-gray-400 mt-1.5">Fecha con la que se nombrará la tabla generada.</p>
                        </div>
                        ...
                        <div>
                            <label for="archivo"
                                class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">
                                Archivo DATOS.CSV (.ZIP) <span class="text-red-500">*</span>
                            </label>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const form = document.getElementById('formCargaMap');
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            Swal.fire({
                                ...
                                text: data.message || 'La carga se realizó correctamente.',
                                icon: 'success',
                                confirmButtonColor: '#9B2242',
                                confirmButtonText: 'Continuar'
                            }).then(() => {
                                form.reset();
                                document.getElementById('lbl_FECHA').value = "{{ date('Y-m-d') }}";
                            });
                        } else {
                            Swal.fire({
                                title: 'Error en el proceso',
                                text: data.message || 'Ocurrió un error al procesar.',
                                icon: 'error',
                                confirmButtonColor: '#9B2242'
                            });
                        }
